...
RSpec.describe 'Followings', type: :system do
  ...
  describe 'feed on Home page' do
    before do
      ...
      create_list(:micropost, 10, user: user.following.first)
      create_list(:micropost, 10)
      visit root_path
    end
    it do
      user.feed.paginate(page: 1).each do |micropost|
        expect(find_by_id("micropost-#{micropost.id}").text).to include CGI.escapeHTML(micropost.content)
      end
    end
  end
end
